...
export function useAuth() {
  ...
  // Clear demo token when Replit session is detected
  useEffect(() => {
    if (user?.authType === 'replit' && hasDemoToken) {
      console.log('🔄 Replit session detected - clearing demo token');
      removeDemoToken();
      setHasDemoToken(false);
      // Refetch to get clean Replit user data without demo token header
      refetch();
    }
  }, [user?.authType, hasDemoToken, refetch]);
